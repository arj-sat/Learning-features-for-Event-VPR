...
def process_csv_file(csv_path: str, filter_instance: FilterDataRecursive, 
    ...
    time_span = (df['timestamp'].max() - df['timestamp'].min())  # μs to seconds
    event_rate = len(df) / time_span
    with open(LOG_FILE, 'a') as f:
        f.write(f" Event rate {event_rate:.1f} events per sec\n") 
    ...
    with open(LOG_FILE, 'a') as f:
        f.write(f" Kept {len(filtered_df)} events {len(filtered_df)/len(df)*100:.1f}%\n)")
